...
void
Board::setCell(uint16_t val, uint16_t row, uint16_t col)
{
  this->board[row][col] = val;
}

uint16_t
Board::getCell(uint16_t row, uint16_t col) const
{
  return (this->board[row][col]);
}

void
Board::clearCell(uint16_t row, uint16_t col)
{
  this->setCell(0, row, col);
}

bool
Board::isPossible(uint16_t val, uint16_t row, uint16_t col) const
{
  // validate args
